...
export type IXSEFVaule = string | number | boolean | IXSEFObjectSection | IXSEFObjectDataSection;
...
export function parse(data: string, params?: IParseConfig): IXSEFObject {
    const { delimiter = "=", comment = ";", nothrow = false, autoTyping = true, dataSections = [] } = { ...params };
...
export function stringify(data: IXSEFObject, params?: IStringifyConfig): string {
    const { delimiter = "=", blankLine = true, spaceBefore = false, spaceAfter = false } = { ...params };
    const chunks: string[] = [];
